...
<template>
  <div class="about">
    ...
    <button @click="$store.commit('increment', 3)">3증가</button>
    <button @click="$store.commit('decrement', 3)">3감소</button>
    <!-- 페이로드 Object -->
    <button @click="$store.commit('objectIncrement', {amount: 10})">오브젝트 10</button>
    <!-- 리셋 -->
    <button @click="$store.commit('resetCount')">리셋</button>
